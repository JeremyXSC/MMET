...
from flava.data import ImageDataModule, MLMDataModule, VLDataModule, MyVLDataModule
...
def main():
    ...
    if "myvl" in config.datasets.selected:
        myvl_datamodule = MyVLDataModule(
            **build_datamodule_kwargs(config.datasets.myvl, config.training)
        )
   
        datamodules.append(myvl_datamodule)
