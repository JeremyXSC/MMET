...
from flava.data.sampler import RandomIdentitySampler
...
class MLMDataModule(TextDataModule):
    ...
    def setup(self, stage=None):
        ...
        self.train_dataset = self.train_dataset.filter(lambda example: example["text"] is not None)

        '''
        import pdb; pdb.set_trace()
        self.train_dataset = self.train_dataset.filter(
            lambda example: "image" in example
        )
        '''
        ...
        self.val_dataset = self.val_dataset.filter(lambda example: example["text"] is not None)
    ...
    def _build_dataloader(self, dataset, drop_last=True, shuffle=False):
        # uneven batches can cause distributed issues,
        # drop last batch to prevent those.
        # ideally, we don't need to drop these for unimodal cases
        # but just to be safe
...
class VLDataModule(LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        now = time.time()
        img = []
        for image_url in train_dataset["image_url"]:
        ...
        # train_dataset["image"] = img
        print((time.time() - now), "s")
        import pdb;pdb.set_trace()
        feature = datasets.Image()
        # assuming image is a Pillow image
        # img = [feature.encode_example(i) for i in img if i is not None]
        now = time.time()
        # img = feature.encode_example(img)
        # img = [feature.encode_example(i) for i in img]
        img = [feature.encode_example(i) if i is not None else None for i in img]
        print((time.time() - now), "s")

        now = time.time()
        train_dataset = train_dataset.add_column(name="image", column=img)
        print((time.time() - now), "s")
        '''
        ...
        train_dataset = train_dataset.map(
            ...
            cache_file_name = "/cluster/home/guanmengyuan/.cache/huggingface/datasets/red_caps/jellyfish/1.0.0/d0d70a901e22f5e3b9a7af1f96f31c6243589705a5ab782b9ac69fcf727d97be/cache-11804e5913e47a28.arrow",
            fn_kwargs={
        ...
        # dir = '/cluster/home/guanmengyuan/multimodal-main/examples/red_caps_download/jellyfish/'
        # train_dataset = train_dataset.map(lambda batch: {"image": [Image.open(dir + image_url).convert('RGB') for image_url in batch["image_url"]]}, batched=True, batch_size=self.fetch_batch_size)
...
class MyVLDataModule(LightningDataModule):
    def __init__(
        self,
        train_infos: List[HFDatasetInfo],
        val_infos: List[HFDatasetInfo],
    ...
    def setup(self, stage=None):
        ...
        # import pdb; pdb.set_trace()
        self.train_dataset_ori = copy.deepcopy(train_dataset)
        self.train_dataset = train_dataset
        self.train_dataset.set_transform(
            partial(
                train_vl_transform,
        ...
        val_dataset = val_dataset.filter(lambda example: example["image"] is not None)
        '''
        val_dataset = val_dataset.filter(lambda example: example["text"] is not None)
        # import pdb; pdb.set_trace()
        self.val_dataset_ori = copy.deepcopy(val_dataset)
        self.val_dataset = val_dataset
        self.val_dataset.set_transform(
            partial(
                val_vl_transform,
    ...
    def train_dataloader(self):
        return torch.utils.data.DataLoader(
            ...
            batch_size=self.batch_size,
            num_workers=self.num_workers,
            # sampler=RandomIdentitySampler(self.train_dataset_ori, self.batch_size, 4),
            sampler=None,
            shuffle=True,#False,
            collate_fn=self._build_collator(),
            # uneven batches can cause distributed issues,
    ...
    def val_dataloader(self):
        return torch.utils.data.DataLoader(
            ...
            batch_size=self.batch_size,
            num_workers=self.num_workers,
            # sampler=RandomIdentitySampler(self.val_dataset_ori, self.batch_size, 1),
            sampler=None,
            shuffle=False,
            collate_fn=self._build_collator(),
            # uneven batches can cause distributed issues,
